...
#SET PATHS AND THEN DEFINE VARS FOR PATHS

# ('/Users/lillyel-said/Desktop/inquisit/pilot_data/rey_ant_pilot2_17_04_01/data_dir/patient_data/rey_patient_data_05_05_17_1.csv')
# ('/Users/lillyel-said/Desktop/inquisit/pilot_data/rey_ant_pilot2_17_04_01/data_dir/patient_data/rey_pilot_04_06_17_parsed_raw_data_patients.csv')
# ('/Users/lillyel-said/Desktop/inquisit/pilot_data/rey_ant_pilot2_17_04_01/data_dir/patient_data/rey_pilot_04_06_17_scored.csv')

all_subj_data_csv =
data_output_raw_csv =
data_output_scored_csv =

with open(all_subj_data_csv,'U') as file:
    input_csv_lines_all_subj = reader(file)
    input_csv_lines_all_subj= map(list, zip(*input_csv_lines_all_subj))          # zip iterates over the csv file, *operator
    all_subj_csv_lines = dict((rows[0],rows[1:]) for rows in input_csv_lines_all_subj)


rey_word_lists ={'rey_list_presentation_1a': ['drum', 'curtain', 'bell','coffee','school','parent','moon','garden', 'hat','farmer','nose','turkey', 'color', 'house', 'river'],

                 'rey_list_presentation_2a': ['pipe', 'wall', 'alarm', 'sugar', 'student', 'mother','star', 'painting', 'bag', 'wheat', 'mouth', 'chicken', 'sound','door', 'stream'],

                 'rey_list_presentation_3a': ['violin', 'tree', 'scarf', 'ham', 'suitcase', 'cousin', 'earth', 'stairs', 'dog', 'banana', 'town', 'radio', 'hunter', 'bucket', 'field'],

                 'rey_list_presentation_4a': ['doll', 'mirror', 'nail', 'sailor', 'heart', 'desert', 'face', 'letter', 'bed', 'machine', 'milk', 'helmet', 'music', 'horse', 'road'],

                 'rey_list_presentation_1b': ['desk', 'ranger','bird','shoe','stove','mountain', 'glasses', 'towel', 'cloud', 'boar','lamb','gun', 'pencil', 'church', 'fish'],

                 'rey_list_presentation_2b': ['bench', 'officer', 'cage', 'sock','fridge','cliff', 'bottle', 'soap', 'sky', 'ship', 'goat','bullet', 'paper', 'chapel', 'crab'],

                 'rey_list_presentation_3b': ['orange', 'table', 'toad', 'corn', 'bus', 'chin', 'bleach', 'soap', 'hotel', 'donkey', 'spider', 'money', 'book', 'soldier', 'padlock'],

                 'rey_list_presentation_4b' : ['dish', 'jester', 'hill', 'coat', 'tool', 'forest', 'perfume', 'ladder', 'girl', 'foot', 'shield', 'pie', 'insect', 'ball', 'car']}
...
for list_type in sorted(index_number_resp.keys()):
    ...
    #print "**************************************************"
    #print list_type
    #print rey_recall_word_count

total_response_for_list=dict()
for list_type in sorted(index_number_resp.keys()):
#     print "**************************************************"
#     print list_type
#     print sorted(set(index_number_resp[list_type]))
...
for word_list in sorted(total_response_for_list.keys()):
    ...
    for word in total_response_for_list[word_list]:
        ...
        print word, max(wordcorrs),rey_word_lists[word_list][wordcorrs.index(max(wordcorrs))]
    # print [[x[0],x[1],x[2]] for x in sorted([[x[1] for x in word_corrs]])]
    # print word_corrs
    # print [x_3 for (x_1,x_2,x_3) in sorted(zip([x[1] for x in word_corrs],[x[0] for x in word_corrs],[x[2] for x in word_corrs]))],rey_word_lists[word_list]
...
for subj in sorted(set(all_subj_csv_lines['subject'])):
    try:
        ...
        print "subject %s has an error in their data" %subj
        continue
...
with open(data_output_raw_csv, 'wb') as csvfile:
    ...
    #                     print [SequenceMatcher(None, response[2], x).ratio() for x in rey_word_lists[list_to_use]]
                    response.append(1)
                else:
                    response.append(0)
