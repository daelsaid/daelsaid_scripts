...
patient_scored = pandas.read_csv(
    '/Volumes/daelsaid/inquisit/INQUISIT_RERUN/all_launches_dir/scored/clean_copies/scored_data_2017_07_01_newageranges_comb_mturk.csv')

patient_scored = pandas.read_csv(
    '/Volumes/daelsaid/inquisit/INQUISIT_RERUN/all_launches_dir/scored/clean_copies/scored_data_2017_07_19_patients_new_ageranges.csv')


test_df = patient_scored.loc[:, 'subj_id':'trial7'].dropna()

patient_testdf = pandas.DataFrame(data=patient_scored.set_index(
    'subj_id').loc[:, 'trial1':'trial7']).dropna().astype(int)

patient_testdf1 = pandas.DataFrame(data=patient_scored.set_index(
...
with PdfPages('/Users/lillyel-said/Desktop/patient_specific_learning_trajectories.pdf') as pdf:
    for index, value in test_df.groupby('subj_id'):
        ...
        fig = patient_testdf.loc[index].astype(float).plot(ax=axes, y=y, subplots=False, fontsize=16, grid=True, yticks=range(0, 16), ylim=(
    ...
    for ix, value in test_df.groupby('subj_id'):
        ...
        learning2 = patient_testdf.loc[(ix)][['trial5', 'trial7']].astype(float).plot(ax=axes2, fontsize=15, subplots=False, grid=True, yticks=range(
    ...
    new_df["diff_7_5"] = patient_testdf["trial7"].sub(
    ...
    merged_df = pandas.merge(new_df, df_with_groupings,
                             on='subj_id', copy=True, indicator=False)
